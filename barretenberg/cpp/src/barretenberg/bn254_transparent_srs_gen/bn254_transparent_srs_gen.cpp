...
const std::string protocol_name = "BARRETENBERG_BN254_IPA_CRS";
/**
 * @brief Generates a monomial basis BN254 SRS (for IPA).
 *
 * @details We only provide functionality to create a single transcript file.
 * ! Note that, unlike the bn254 (KZG) SRS, the first element of the bn254_ipa SRS will not be equal to point one
 * defined in fr.hpp as this function finds bn254 points in an arbitrary order.
 *
 */
int main(int argc, char** argv)
{
    ...
    const std::string srs_path = (args.size() > 2) ? args[2] : "../srs_db/bn254/";
    ...
    std::vector<bb::g1::affine_element> srs(subgroup_size);
    ...
    parallel_for_range(subgroup_size, [&](size_t start, size_t end) {
        ...
        for (size_t point_idx = start; point_idx < end; ++point_idx) {
            ...
            while (!rational_point_found) {
                hash_input.clear();
                // We hash
                // |BARRETENBERG_BN254_IPA_CRS|POINT_INDEX_IN_LITTLE_ENDIAN|POINT_ATTEMPT_INDEX_IN_LITTLE_ENDIAN|
                ...
                auto crs_element = bb::g1::affine_element::from_compressed(hash_result_uint);
                ...
                if (!crs_element.x.is_zero() || !crs_element.y.is_zero()) {
                    rational_point_found = true;
                    {
                        ...
                        srs.at(point_idx) = static_cast<bb::g1::affine_element>(crs_element);
                    }
                    break;
                }
                attempt += 1;
    ...
    bb::srs::IO<bb::curve::BN254>::write_transcript(&srs[0], manifest, srs_path);
